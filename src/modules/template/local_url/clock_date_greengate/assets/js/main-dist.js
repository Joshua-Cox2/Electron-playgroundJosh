...
function loadClientData(clientData) {
    $('body').width(displayWidth);
    $('body').height(displayHeight);
    if (displayWidth < displayHeight) {
        $('#main').addClass('portDisp');
    }
    var fontSize = Math.round(displayHeight / 2.7);
    $('body').css('font-size', fontSize + 'px');
};

function retrieveJson(scriptPath) {
    var weatherJson = document.createElement("script");
    weatherJson.setAttribute("src", 'http://iris.lymlive.com.au/publish/weatherData/' + scriptPath);
    // "?callback=?"
...
function withinIframe() {
    if (window.frameElement) {
        topElement = window.frameElement;
        return true;
    }
    return false;
}
...
function setDisplaySettings() {
    if (notTop) {
        displayWidth = topElement.clientWidth;
        displayHeight = topElement.clientHeight;
    } else {
        displayWidth = clientData["screen"]["screen_width"];
        displayHeight = clientData["screen"]["screen_height"];
    }
}
...
function displayCurrentDateTime() {
    currentTime = moment();
    $("#clock").html(currentTime.format("hh:mm A"));
    $('#date').html(currentTime.format("dddd Do MMMM, Y"));
    loopDateTime();
}

function loopDateTime() {
    var waitLoop = setTimeout(function() {
        displayCurrentDateTime();
    }, 100);
}

//@prepros-prepend jquery/jquery-2.1.4.js
//@prepros-prepend bootstrap/anchor.min.js
//@prepros-prepend bootstrap/clipboard.min.js
//@prepros-prepend bootstrap/holder.min.js
//@prepros-prepend bootstrap/popper.min.js
//@prepros-prepend bootstrap/bootstrap.js
//@prepros-prepend moment/moment.js
//@prepros-prepend variables.js
